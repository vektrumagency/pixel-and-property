...
export function DigitalServices() {
  const t = useTranslations("digital.services");
      ...
      <div className="grid gap-x-12 gap-y-12 lg:grid-cols-3 lg:gap-y-16">
        {items.map((item, i) => (
          <Reveal key={item.name} delay={(i % 3) * 100} className="relative">
            {item.tag && (
              <span className="mb-4 inline-block border border-gold/40 px-2 py-0.5 text-[0.48rem] uppercase tracking-[0.2em] text-gold-dark">
                {item.tag}
              </span>
            )}
            <span className="mb-4 block h-px w-8 bg-gold-dark/60" />
            <h3 className="mb-3 font-serif text-xl font-normal">{item.name}</h3>
            <p className="text-[0.75rem] leading-[1.9] text-black/50">
              {item.desc}
            </p>
          </Reveal>
        ))}
